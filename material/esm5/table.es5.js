...
MatTableDataSource = /** @class */ (function (_super) {
    ...
    function MatTableDataSource(initialData) {
        ...
        _this.sortData = function (data, sort) {
            var _a;
            /** @type {?} */
            var active = Array.isArray(sort.active) ? sort.active : [sort.active];
            if (!active[0]) {
                return data;
            }
            /** @type {?} */
            var direction = typeof sort.direction !== 'object' ? (_a = {}, _a[active[0]] = sort.direction, _a) :
                sort.direction;
            if (!direction) {
                return data;
            }
            return data.sort(function (a, b) {
                ...
                return active.reduce(function (previous, sortId) {
                    if (previous !== 0) {
                        return previous;
                    }
                    /** @type {?} */
                    var valueA = _this.sortingDataAccessor(a, sortId);
                    /** @type {?} */
                    var valueB = _this.sortingDataAccessor(b, sortId);
                    return _this.compareValues(valueA, valueB, direction[sortId]);
                }, 0);
            });
        };
        /**
     ...
     * @return {?}
     */
    MatTableDataSource.prototype.compareValues = 
    // If both valueA and valueB exist (truthy), then compare the two. Otherwise, check if
    // one value exists while the other doesn't. In this case, existing value should come first.
    // This avoids inconsistent results when comparing values to undefined/null.
    // If neither value exists, return 0 (equal).
     ...
     * @return {?}
     */
    function (valueA, valueB, direction) {
        /** @type {?} */
